...
BLUE, SKY, AMBER, CORAL, GREY = "#1F6FB2", "#5DA3D5", "#E0A93E", "#C0392B", "#9AA5B1"
...
fig1 = go.Figure()
for i, (reg, rate, col) in enumerate(zip(REGIONS, RATES, [BLUE, SKY, AMBER, None])):
    if col:
...
for reg, a, l in zip(order, ages_o, lives_o):
    ...
    fig3.add_annotation(x=(a+l)/2, y=reg, text=f"<b>+{l-a:.0f} yrs</b>",
                        showarrow=False, yshift=14, font=dict(size=11, color=CORAL))
...
fig3.add_trace(go.Scatter(x=lives_o, y=order, mode="markers",
                          ...
                          marker=dict(size=13, color=CORAL),
...
fig4.add_bar(x=SG_MIRROR.Year, y=SG_MIRROR["SG-reported exports"],
             name="Singapore reports exporting", marker_color=BLUE, width=0.35,
             offsetgroup=0)
fig4.add_bar(x=SG_MIRROR.Year, y=SG_MIRROR["Partner-recorded imports"],
             name="Partners record importing", marker_color=CORAL, width=0.35,
             offsetgroup=1)
